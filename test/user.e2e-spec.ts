...
describe("UsersController (unit)", () => {
  ...
  const updateUserUseMock = { execute: jest.fn().mockResolvedValue({ ...mockUser, firstName: 'James' }) };
  const archiveUserUseMock = { execute: jest.fn().mockResolvedValue({ ...mockUser, isArchived: true }) };
  ...
  beforeAll(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        { provide: UserUseCase.GetUserByIdUseCase, useValue: getUserByIdUseMock },
        { provide: UserUseCase.UpdateUserUseCase, useValue: updateUserUseMock },
        { provide: UserUseCase.ArchiveUserUseCase, useValue: archiveUserUseMock },
      ],
      exports: [],
    }).compile();
  ...
  it("getUserById should throw NotFoundException when user not found", async () => {
    ...
    getUserByIdUseMock.execute.mockRejectedValueOnce(new NotFoundException('User not found'));
    await expect(controller.getUserById(dto)).rejects.toThrow(NotFoundException);
    expect(getUserByIdUseMock.execute).toHaveBeenCalledWith(dto);
  });
  ...
  it('updateUser should update and return the updated user', async () => {
    const dto: UserDto.UpdateUserDto = { firstName: 'James', lastName: '', email: mockUser.email };
  ...
  it('updateUser should propagate errors from use case', async () => {
    const dto: UserDto.UpdateUserDto = { firstName: 'X' };
    updateUserUseMock.execute.mockRejectedValueOnce(new Error('Invalid'));
    await expect(controller.updateUser(mockUser.id, dto)).rejects.toThrow('Invalid');
    expect(updateUserUseMock.execute).toHaveBeenCalledWith(mockUser.id, dto);
  });
  ...
  it('archiveUser should throw NotFoundException when archiving missing user', async () => {
    ...
    archiveUserUseMock.execute.mockRejectedValueOnce(new NotFoundException('Archived user not found.'));
    await expect(controller.archiveUser(dto)).rejects.toThrow(NotFoundException);
    expect(archiveUserUseMock.execute).toHaveBeenCalledWith(dto);
  });
});